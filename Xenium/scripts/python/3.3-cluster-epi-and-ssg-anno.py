...
cluster_xenium(adata_epi.copy(), nns, pcs, key="epithelial",output_path=f"{adata_output_dir}/epithelial_{nns}_{pcs}_harmony_batch_05_pt_05_ssg.h5ad", scale=scale, hvgs=hvgs, harmony=["batch","patient_id"], thetas=[0.5,0.5])


senepy_high = adata_epi[adata_epi.obs["senepy_high"] == True].copy()
senepy_high.write_h5ad(f"{adata_output_dir}/senepy_high.h5ad")
